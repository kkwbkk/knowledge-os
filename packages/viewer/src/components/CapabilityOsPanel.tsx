...
import type {
  ViewerCapabilityAdmission,
  ViewerCapabilityBlindArtifact,
  ViewerCapabilityOsArtifact,
  ViewerCapabilityRecord,
  ViewerCapabilityRelation
} from "../lib";
import { CapabilityBlindTest } from "./CapabilityBlindTest";

type CapabilityOsPanelProps = {
  artifact: ViewerCapabilityOsArtifact | null;
  blindArtifact?: ViewerCapabilityBlindArtifact | null;
  error?: string;
  blindError?: string;
};
...
export function CapabilityOsPanel({ artifact, blindArtifact = null, error, blindError }: CapabilityOsPanelProps) {
  const [view, setView] = useState<"objects" | "blind">("objects");
  ...
  return (
    <div className="capability-os-panel">
      ...
      <div className="capability-view-switch" role="tablist" aria-label="Knowledge OS view">
        <button
          type="button"
          role="tab"
          className={`btn${view === "objects" ? " is-active" : ""}`}
          aria-selected={view === "objects"}
          onClick={() => setView("objects")}
        >
          规范对象 {artifact.records.length}
        </button>
        <button
          type="button"
          role="tab"
          className={`btn${view === "blind" ? " is-active" : ""}`}
          aria-selected={view === "blind"}
          onClick={() => setView("blind")}
        >
          盲测 {blindArtifact?.questionCount ?? 0}
        </button>
      </div>

      {view === "blind" ? (
        <CapabilityBlindTest key={blindArtifact?.sourceHash ?? "empty"} artifact={blindArtifact} error={blindError} />
      ) : (
        <>
